...
class FastaFile(dict):
    ...
    def __init__(self, fileName):
        super(dict)
        with open(fileName,'r') as self.fileHandler:
            self.__parse()
    ...
    def renameHeaders(self, old, new):
        for i in self.keys():
            self[i.replace(old, new)] = self.pop(i)
...
class FastqFile(dict):
    ...
    def __init__(self, fileName):
        ...
        while True:
            ...
            self[name] = FastqEntry(name, seq, qual, seqLen)
    
    def toString(self, start=0, end = None):
        buffer = []
        for key in self:
            buffer.append(self[key].toString(start, end))
        return "".join(buffer)
            
class FastqEntry():
    
    def __init__(self, name, seq, qual, seqLen):
        self.name = name
        self.seq = str(seq)
    ...
    def toString(self, start=0, end = None):
        """
        Trim before returning entry as a string
        """
...
class GFF3File(dict):
    ...
    def __parse(self):
        ...
        class DATA:
            """
                DATA storage format of GFF3
            """
            __slots__ = ("seqname", "source", "feature", "start", "end", 
        ...
        count = 0
        for l in self.fileHandler:
            if l.startswith('##FASTA'):
                break
            if not l.startswith('##'):
                count += 1
                elements = l.split('\t')
                self[count] = DATA(*elements) # fill data
                
    def get_feature_numbers(self):
        """
        Returns the count of all features as dictionary
        """
        from collections import Counter
        out = []
        for count, data in self.items():
            out.append(data.feature)
        return Counter(out)
    
    def counts(self, feature):
        """
        Returns number of annotations of feature
        """
        return self.get_feature_numbers()[feature]
    ...
    def get_line(self):
        """
        Yields the line of annotation in DATA format
        """
        for i in self.values():
            yield i
...
class SAMFile(dict):
    ...
    def __parse(self):
        ...
        class DATA:
            """
                DATA storage format of GFF3
            """
            __slots__ = ("qname", "flag", "rname", "pos", "mapq", 
                         "cigar", "rnext", "pnext", "tlen", "seq", "qual", "tags")
            
            def __init__(self, qname, flag, rname, pos, mapq, 
                         cigar, rnext, pnext, tlen, seq, qual, tags):
                self.qname = qname
                self.flag = flag
                self.rname = rname
        ...
        for l in self.fileHandler:
            if l.startswith('@'):
                ...
                l = l.strip('\n')
                elements = l.split('\t')
                elements = elements[0:11] + [elements[11:]] # combine tags
                self[count] = DATA(*elements) # fill data
    
    def nr_total_mappings(self):
        """
        Returns number of total mappings
        """
        return len(self.keys())
    
    def nr_unique_mappings(self):
        """
        Returns number of unique reads mapped
        """
        collect = set()
        for i in self.values():
            collect.add(i.qname)
    ...
    def count_perfect_mapped(self, seqsize):
        """
            Returns number of perfect mapped reads
        """
        count = 0
        for i in self.values():
            if 'MD:Z:%s' % (seqsize) in i.tags:
                count += 1
        return count
    
    def get_perfect_mapped(self, seqsize):
        """
            Returns number of perfect mapped reads
        """
        for i, v in self.items():
            if 'MD:Z:%s' % (seqsize) in v.tags:
                yield self[i]
